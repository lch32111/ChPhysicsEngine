#pragma once
#ifndef __CH_BROAD_PHASE_H__
#define __CH_BROAD_PHASE_H__
...
namespace Chan
{
	...
	public:
		...
		void UpdateProxy(int proxyId, const c2AABB& aabb, const ChVector2& displacement);
		void UpdateProxy(int proxyId, const c2AABB& aabb);
	...
	inline void CGBroadPhase::UpdatePairs(T * callback)
	{
		...
		for (int i = 0; i < m_moveCount; ++i)
		{
			...
			const c2AABB& fatAABB = m_tree.GetFatAABB(m_queryProxyId);
	...
	template<typename T>
	inline void CGBroadPhase::Query(T * callback, const c2AABB & aabb) const
	{
		m_tree.Query(callback, aabb);
	}

	template<typename T>
	inline void CGBroadPhase::RayCast(T * callback, const c2RayInput & input) const
	{
...
#undef BROAD_RESULT_WRAPPER_INIT_SIZE
	...
	public:
		...
		void setColor(const ChVector2& leaf, const ChVector2& branch)
		{
			leafColor = leaf;
			branchColor = branch;
		}
	...
	private:
		...
		ChVector2 leafColor;
		ChVector2 branchColor;
		float leafWidth = 1.f;
		float branchWidth = 1.f;

		void recursiveDraw(int index);
		void render(const c2AABB& aabb, const ChVector2& Color, float lineWidth);
	};
	*/
	...
	struct CGRayCastCallback
	{
		...
		(
			const c2RayOutput& output,
			const c2RayInput& input,
			void* userData
		) = 0; // interface
	};
	...
	public:
		virtual bool RayCastCallback(
			const c2RayInput& input, int nodeId) = 0; // Interface
	...
	{
	public:
		bool RayCastCallback(const c2RayInput& input, int nodeId)
		{
